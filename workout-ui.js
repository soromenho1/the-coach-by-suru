...
window.mountCoachWorkouts = function(container, {client, account, studentId, valid, back}) {
  ...
  const form=(kind,title,fields,parent='')=>`<form class="card" data-kind="${kind}" data-parent="${esc(parent)}" data-draft="${window.crypto.randomUUID()}" novalidate><h3>${title}</h3>${fields}<div class="w-message" role="status" aria-live="polite" tabindex="-1"></div><button type="submit" class="btn green">${kind==='finish'?'Concluir treino':kind==='set'?'Guardar série':'Guardar'}</button></form>`;
  ...
  function previous(){if(route.type==='plans')return back();if(route.type==='plan'||route.type==='history')return show({type:'plans'});if(route.type==='workout')return show({type:'plan',id:ctx.plan.id});if(route.type==='session')return show({type:'history'});}
  ...
  function sessionHtml(data,history){
    ...
    for(const e of exercises){
      html+=exerciseCard(e,history);
      if(writable){
        for(let n=1;n<=e.sets&&n<=100;n++){
          const log=data.logs.find(l=>l.exercise_id===e.id&&l.set_number===n);
          html+=log?savedSet(log,e,history,data.session.id):form('set',`Série ${n} · ${esc(e.name)}`,pair(field('weight','Carga (kg)','','text','decimal'),field('reps','Repetições','','text','numeric'))+field('rir','RIR (opcional)','','text','decimal')+notes()+`<input type="hidden" name="exercise_id" value="${esc(e.id)}"><input type="hidden" name="set_number" value="${n}">`,data.session.id);
        }
    ...
    if(writable&&exercises.length)html+=form('finish','Concluir sessão',notes('Notas da sessão',data.session.notes||'')+'<p class="muted">A sessão só é concluída depois de todas as séries estarem guardadas.</p>',data.session.id);
  ...
  async function show(next,message=''){
    ...
    try{
      ...
      if(next.type==='plans'){
        ...
        html=`<h2>${esc(data.workout.name)}</h2>`+(data.exercises.length?data.exercises.map(e=>exerciseCard(e,history)).join(''):'<div class="card">Ainda não existem exercícios neste treino.</div>')+(coach?exerciseForm(next.id):'');
  ...
  container.addEventListener('click',async event=>{
    ...
    try{
      ...
      if(action==='start'&&student){const session=await W.start(client,account,studentId,id,ctx.startId,today(),live);if(live())await show({type:'session',id:session.id});}
  ...
  container.addEventListener('submit',async event=>{
    const f=event.target;if(!f.dataset.kind)return;event.preventDefault();if(!valid()||busy.has(f))return;
    ...
    try{
      ...
      if(kind==='plan')result=await W.createPlan(client,account,studentId,v,f.dataset.draft,live);
        ...
        result=await W.saveSet(client,account,studentId,parent,v.exercise_id,Number(v.set_number),v,id,live);
        if(live()){
          const exercise=ctx.detail.exercises.find(e=>e.id===v.exercise_id);ctx.logs.push(result.row);
          f.outerHTML=savedSet(result.row,exercise,ctx.history,parent)+(result.recovered?notice('Esta série já estava guardada. Mantivemos o registo existente.'):notice('Série guardada com sucesso.'));
        }return;
      }else if(kind==='finish'){
    ...
    finally{busy.delete(f);submit.disabled=false;}
  });
  show({type:'plans'});
};
